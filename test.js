(function () {
    ...
    // Функция получения данных (PlaybackInfo для субтитров + поток)
    ...
    function startPlugin() {
        // --- ВНЕДРЕНИЕ В НАСТРОЙКИ ---
        // Создаем кнопку для главного меню настроек
        var settings_item = $('<div class="settings-folder selector" data-component="jellyfin_nas">' +
            '<div class="settings-folder__icon">' + icon + '</div>' +
            '<div class="settings-folder__name">Jellyfin NAS</div>' +
            '<div class="settings-folder__descr">Ваша медиатека с субтитрами</div>' +
        '</div>');
        ...
        // Слушаем открытие настроек и втыкаем кнопку
        Lampa.Listener.follow('activity', function (e) {
            if (e.type == 'render' && e.component == 'settings') {
                var container = e.render.find('.settings-list');
                if (!container.find('[data-component="jellyfin_nas"]').length) {
                    container.append(settings_item);
                }
            }
        });
